...
#####################################
# Literature Miner Untility Functions
...
pubmed_miner <- function(UniProtID, IDType, taxid, keyword, ti.only, query.idx=1) {		
	...
	#		list.terms = list(synonyms)
			...
			# pubmed.result <- pubmed_summary(list.terms, vec.keyword, fields)
  ...
  uniprot.res = try(GET(query))
	...
	if(!inherits(uniprot.res, "try-error"))
	 if(!is.null(httr::content(uniprot.res)))
	    #list.syn = unique( strsplit(uniprot.res, split="\n")[[1]][-1])
...
plot_stats <- function(dat.pubmed, file='barplotNwordcloud.png') {		
	...
	# list.years <- list(dat.pubmed$Year, dat.pubmed$Year[dat.pubmed$ConditionInTitle==TRUE], 
		# dat.pubmed$Year[dat.pubmed$IsReview==TRUE])
	
	# label = c('Overall', 'Condition in title', 'Review')
	...
	wcl = try(wordcloud(all.abstract, max.words=200))
